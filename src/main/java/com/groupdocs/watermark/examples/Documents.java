...
public class Documents {
	public static class PDF {
		...
		private final static String FILE_PATH = "input.pdf";
		// ExEnd:SourcePDFFilePath "山西省农村信用社.*"
...
//		}
		 ...
		 */
		public static void removeWatermark(String STORAGE_PATH, String OUTPUT_PATH, String FILE_NAME, String watermark) {
			try {
				// ExStart:RemoveWatermarkFromPDF
				PdfDocument doc = Document.load(PdfDocument.class, Common.mapSourceFilePath(STORAGE_PATH, FILE_NAME));

				// Initialize search criteria
				ImageSearchCriteria imageSearchCriteria = new ImageDctHashSearchCriteria(Common.WATERMARK_IMAGE_PATH);
				//TextSearchCriteria textSearchCriteria = new TextSearchCriteria("This is a test watermark") 山西省农村信用社.*;

				Pattern pattern = Pattern.compile(watermark);

				for (int k = 0; k < doc.getPages().getCount(); k++) {

					TextSearchCriteria textSearchCriteria = new TextSearchCriteria(pattern);

					PossibleWatermarkCollection possibleWatermarks = doc.getPages().get_Item(k)
				...
				doc.save(Common.mapOutputFilePath(OUTPUT_PATH, FILE_NAME));
				doc.close();
...
//					if ("锟� Aspose 2016".equals(shape.getText())) {
//						shape.setText("锟� GroupDocs 2017");
//					}
//				}
//
//				doc.save(Common.mapOutputFilePath(FILE_PATH));
...
//						shape.getFormattedTextFragments().add("锟� GroupDocs 2017",
...
//					if (shape.getText() != null && shape.getText().contains("锟� Aspose 2016")) {
//						shape.setText("锟� GroupDocs 2017");
//					}
//				}
//
